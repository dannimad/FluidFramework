...
describe("SessionIdNormalizer", () => {
	...
	it("fails when adding finals out of order", () => {
		...
		assert.throws(
			...
			(e: any) => validateAssertionError(e, "Malformed normalization range."),
		);
	});
	...
	it("fails when registering final blocks with an invalid count", () => {
		...
		assert.throws(
			...
			(e: any) => validateAssertionError(e, "Malformed normalization block."),
		);
		assert.throws(
			() => {
	...
	it("fails when gaps in finals do not align with a local", () => {
		...
		assert.throws(
			...
			(e: any) => validateAssertionError(e, "Gaps in final space must align to a local."),
		);
	});
