...
describe("IdCompressor", () => {
	it("detects invalid cluster sizes", () => {
		...
		assert.throws(
			...
			(e: any) => validateAssertionError(e, "Clusters must have a positive capacity"),
		);
		assert.throws(
		...
		assert.throws(
			...
			(e: any) => validateAssertionError(e, "Clusters must not exceed max cluster size"),
		);
	});
	...
	describe("ID Generation", () => {
		...
		it("will not decompress IDs it did not compress", () => {
			...
			assert.throws(
				...
				(e: any) => validateAssertionError(e, errorMessage),
			);
		});
	...
	describe("Finalizing", () => {
		...
		it("prevents attempts to finalize ranges twice", () => {
			...
			assert.throws(
				() => rangeCompressor.finalizeCreationRange(batchRange),
				(e: any) => validateAssertionError(e, "Ranges finalized out of order."),
			);
	...
	describeNetwork("Networked", (itNetwork) => {
		...
		itNetwork(
			...
			(network) => {
				...
				assert.throws(getSessionNormalizedId, (e: any) =>
					validateAssertionError(
						e,
						"No IDs have ever been finalized by the supplied session.",
					),
			...
			itNetwork(
				...
				(network) => {
					...
					assert.throws(
						...
						(e: any) => validateAssertionError(e, "Cannot resume existing session."),
					);
				},
			);
