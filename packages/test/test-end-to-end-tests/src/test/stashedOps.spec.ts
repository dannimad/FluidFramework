...
describeNoCompat("stashed ops", (getTestObjectProvider) => {
	...
	it("online blob upload", async function () {
		const container = await provider.loadTestContainer(testContainerConfig);
		const dataStore = await requestFluidObject<ITestFluidObject>(
			container,
			"default",
		...
		if (dataStore.runtime.createAndUseBlob) {
			await dataStore.runtime.createAndUseBlob(
				stringToBuffer("testString", "utf-8"),
				(handle2: any) => map.set("testKey", handle2),
			);
		}
		...
		await provider.ensureSynchronized();
		assert.strictEqual(
			bufferToString(await map2.get("testKey").get(), "utf8"),
			"testString",
		);
		assert.strictEqual(
			bufferToString(await map2.get("blob handle").get(), "utf8"),
	...
	it("it can reapplied callback in blobs", async function () {
		const container = await provider.loadTestContainer(testContainerConfig);
		const dataStore = await requestFluidObject<ITestFluidObject>(
			container,
			"default",
		...
		let handle;
		if (dataStore.runtime.createAndUseBlob) {
			handle = dataStore.runtime.createAndUseBlob(
				stringToBuffer("testString", "utf-8"),
				(handle2: any) => map.set("testKey", handle2),
			);
		}

		const stashedChanges = await container.closeAndGetPendingLocalState();
		// await handle;

		const container2 = await loader.resolve({ url }, stashedChanges);
		const dataStore2 = await requestFluidObject<ITestFluidObject>(container2, "default");
		const map2 = await dataStore2.getSharedObject<SharedMap>(mapId);
